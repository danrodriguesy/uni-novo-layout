    ...
    <div className={styles.megaMenuContainer}>
      <div className={styles.menuLeft}>
        {categorias.map((cat, index) => (
          <div
            key={index}
            className={`${styles.menuLeftItem} ${activeIndex === index ? styles.active : ""}`}
            onMouseEnter={() => setActiveIndex(index)}
          >
            <div className={styles.logoWrap}>
              <img src={cat.logo} alt={cat.nome} className={styles.logo} />
              <div className={styles.arrow}><FaCaretRight /></div>
            </div>
      ...
      <div className={styles.menuRight}>
        <h3 className={styles.menuRightTitle}>{categorias[activeIndex].nome}</h3>
        <div className={styles.menuGrid}>
          {categorias[activeIndex].subcategorias.map((item, i) => (
